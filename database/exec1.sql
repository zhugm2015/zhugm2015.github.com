...
SELECT * FROM scott.emp WHERE year(HIREDATE)=1981;
SELECT * FROM scott.emp WHERE (year(localtime)-year(HIREDATE))>12;
# 11、找出姓名以A、B、S开始的员工信息。
SELECT *
FROM scott.emp
WHERE ENAME LIKE '[ABS]%';
# 12、找到名字长度为7个字符的员工信息。
SELECT ENAME FROM scott.emp WHERE CHAR_LENGTH(ENAME)=5;
# 13、名字中不包含R字符的员工信息。
SELECT *
FROM scott.emp WHERE ENAME NOT LIKE '%R%';
# 14、找出员工名字的前3个字符。
...
SELECT ENAME,YEAR(HIREDATE),MONTH(HIREDATE) FROM scott.emp ORDER BY MONTH(HIREDATE),HIREDATE;
# 21、计算员工的日薪(按30天)。
ALTER TABLE scott.emp ADD Wage DOUBLE;
UPDATE scott.emp SET Wage = SAL DIV 30;
# 22、找出2月份雇佣的员工。
SELECT * FROM scott.emp WHERE MONTH(HIREDATE)=02;
# 23、至今为止，员工被雇佣的天数。
# 24、找出姓名中包含A的员工信息。
